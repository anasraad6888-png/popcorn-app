...
export default function HorrorMoviesPage() {
  ...
  const tRows = useTranslations('Rows'); // لاستخدام كلمة "Horror"
  ...
  // دالة جلب أفلام الرعب
  const fetchHorrorMovies = async (pageNum: number) => {
    try {
      const lang = locale === 'ar' ? 'ar-SA' : 'en-US';
      
      // 💡 الرقم 27 هو كود تصنيف "الرعب" في TMDB
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=${lang}&with_genres=27&sort_by=popularity.desc&page=${pageNum}&include_adult=false`
      );
  ...
  useEffect(() => {
    const initialFetch = async () => {
      setLoading(true);
      const newMovies = await fetchHorrorMovies(1);
      setMovies(newMovies);
      setLoading(false);
    };
    initialFetch();
  ...
  const handleLoadMore = async () => {
    ...
    const newMovies = await fetchHorrorMovies(nextPage);
