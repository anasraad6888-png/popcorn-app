...
interface Movie {
  id: number;
  title: string; // الأفلام تستخدم title غالباً
  poster_path: string;
  vote_average: number;
  release_date: string; // الأفلام تستخدم release_date
}

export default function MoviesPage() {
  const locale = useLocale();
  const t = useTranslations('Sidebar'); // لاستخدام كلمة "Movies"
  ...
  const fetchMovies = async (pageNum: number) => {
    try {
      ...
      // discover/movie لجلب الأفلام
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=${lang}&sort_by=popularity.desc&page=${pageNum}&include_adult=false`
      );
    ...
    <div className="min-h-screen bg-[#141414] text-white p-6 md:p-12 md:ps-24 md:pt-24 pt-24">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-12">
        {movies.map((item, index) => (
          <Link 
            key={`${item.id}-${index}`} 
            href={`/watch/${item.id}?type=movie`} // 👈 تحديد النوع كفيلم
