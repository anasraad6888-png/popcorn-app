...
export default function ComedyMoviesPage() {
  ...
  const tRows = useTranslations('Rows'); // لاستخدام كلمة "Comedy"
  ...
  // دالة جلب أفلام الكوميديا
  const fetchComedyMovies = async (pageNum: number) => {
    try {
      const lang = locale === 'ar' ? 'ar-SA' : 'en-US';
      
      // 💡 الرقم 35 هو كود تصنيف "الكوميديا" في TMDB
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=${lang}&with_genres=35&sort_by=popularity.desc&page=${pageNum}&include_adult=false`
      );
  ...
  useEffect(() => {
    const initialFetch = async () => {
      setLoading(true);
      const newMovies = await fetchComedyMovies(1);
      setMovies(newMovies);
      setLoading(false);
    };
    initialFetch();
  ...
  const handleLoadMore = async () => {
    ...
    const newMovies = await fetchComedyMovies(nextPage);
