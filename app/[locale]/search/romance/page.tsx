...
export default function RomanceMoviesPage() {
  ...
  const tRows = useTranslations('Rows'); // لاستخدام كلمة "Romance"
  ...
  // دالة جلب أفلام الرومانسية
  const fetchRomanceMovies = async (pageNum: number) => {
    try {
      const lang = locale === 'ar' ? 'ar-SA' : 'en-US';
      
      // 💡 الرقم 10749 هو كود تصنيف "الرومانسية" في TMDB
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=${lang}&with_genres=10749&sort_by=popularity.desc&page=${pageNum}&include_adult=false`
      );
  ...
  useEffect(() => {
    const initialFetch = async () => {
      setLoading(true);
      const newMovies = await fetchRomanceMovies(1);
      setMovies(newMovies);
      setLoading(false);
    };
    initialFetch();
  ...
  const handleLoadMore = async () => {
    ...
    const newMovies = await fetchRomanceMovies(nextPage);
