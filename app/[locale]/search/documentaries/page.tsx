...
export default function DocumentariesPage() {
  ...
  const tRows = useTranslations('Rows'); // لاستخدام كلمة "Documentaries"
  ...
  // دالة جلب الوثائقيات
  const fetchDocumentaries = async (pageNum: number) => {
    try {
      const lang = locale === 'ar' ? 'ar-SA' : 'en-US';
      
      // 💡 الرقم 99 هو كود تصنيف "الوثائقيات" في TMDB
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=${lang}&with_genres=99&sort_by=popularity.desc&page=${pageNum}&include_adult=false`
      );
  ...
  useEffect(() => {
    const initialFetch = async () => {
      setLoading(true);
      const newMovies = await fetchDocumentaries(1);
      setMovies(newMovies);
      setLoading(false);
    };
    initialFetch();
  ...
  const handleLoadMore = async () => {
    ...
    const newMovies = await fetchDocumentaries(nextPage);
